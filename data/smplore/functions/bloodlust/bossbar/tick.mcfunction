...
### Execute: N/A
# ----------------------------------------------------------------
tag @a remove smplore.bloodlust.boss

## Give smplore.bloodlust.boss tag to the player with the highest bloodlust (minimum of 10)
# ----------------------------------------------------------------
scoreboard players reset $temporary w
scoreboard players operation $temporary w > @a bloodlust
execute as @a[scores={bloodlust=10..}] if score @s bloodlust = $temporary w run tag @s add smplore.bloodlust.boss
...
# ----------------------------------------------------------------
execute as @a[tag=smplore.bloodlust.boss, limit=1] if entity @s run function smplore:bloodlust/bossbar/exists
execute unless entity @a[tag=smplore.bloodlust.boss, limit=1] run function smplore:bloodlust/bossbar/none